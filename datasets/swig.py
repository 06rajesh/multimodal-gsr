...
class CSVDataset(Dataset):
    ...
    def __init__(self, img_folder, ann_file, class_list, verb_path, role_path, verb_info, is_training, transform=None, dataset='swig'):
        ...
        self.captions = self._read_captions(self.SWiG_json)
    ...
    def __getitem__(self, idx):
        ...
        sample = {'img': img, 'annot': annot, 'caption': caption, 'img_name': self.image_names[idx], 'verb_idx': verb_idx, 'verb_role_idx': verb_role_idx}
        if self.transform:
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=300, max_side=400):
        ...
        return {'img': torch.from_numpy(new_image), 'annot': torch.from_numpy(annots), 'caption': sample['caption'], 'scale': scale, 'img_name': image_name, 'verb_idx': sample['verb_idx'], 'verb_role_idx': sample['verb_role_idx'], 'shift_1': shift_1, 'shift_0': shift_0}
...
class Augmenter(object):
    def __call__(self, sample, flip_x=0.5):
        ...
        sample = {'img': image, 'annot': annots, 'caption': sample['caption'], 'img_name': img_name, 'verb_idx': sample['verb_idx'], 'verb_role_idx': sample['verb_role_idx']}
...
class Normalizer(object):
    ...
    def __call__(self, sample):
        image, annots, caption = sample['img'], sample['annot'], sample['caption']

        return {'img': ((image.astype(np.float32) - self.mean) / self.std), 'annot': annots, 'caption': caption, 'img_name': sample['img_name'], 'verb_idx': sample['verb_idx'], 'verb_role_idx': sample['verb_role_idx']}
